...
/**
 * @file <argos3/plugins/robots/foot-bot/control_interface/ci_newepuck_base_ground_sensor.cpp>
 *
...
#include "ci_newepuck_base_ground_sensor.h"
...
namespace argos {
   ...
   CCI_NewEPuckBaseGroundSensor::CCI_NewEPuckBaseGroundSensor() :
      m_tReadings(8) {
   ...
   const CCI_NewEPuckBaseGroundSensor::TReadings& CCI_NewEPuckBaseGroundSensor::GetReadings() const {
     return m_tReadings;
   }
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_NewEPuckBaseGroundSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_NewEPuckBaseGroundSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_NewEPuckBaseGroundSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_NewEPuckBaseGroundSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
