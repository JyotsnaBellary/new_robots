/**
 * @file <newepuck/simulator/dynamics3d_newepuck_model.cpp>
 *
 * @author Michael Allwright - <allsey87@gmail.com>
 */

#include "dynamics3d_newepuck_model.h"
...
namespace argos {
   ...
   CDynamics3DNewEPuckModel::CDynamics3DNewEPuckModel(CDynamics3DEngine& c_engine,
                                                CNewEPuckEntity& c_newepuck) :
      /* technically, the CDynamics3DMultiBodyObjectModel should be initialized with 7 children
         links, however, setting it to 7 makes the newepuck less stable for reasons. */
      CDynamics3DMultiBodyObjectModel(c_engine, c_newepuck, 3, false),
      m_cWheeledEntity(c_newepuck.GetWheeledEntity()) {
      ...
      const SAnchor& sOriginAnchor = c_newepuck.GetEmbodiedEntity().GetOriginAnchor();
      ...
      SAnchor* psBodyAnchor = &c_newepuck.GetEmbodiedEntity().AddAnchor("body", {0.0, 0.0, 0.00125});
      /* create the bodies */
   ...
   void CDynamics3DNewEPuckModel::Reset() {
   ...
   void CDynamics3DNewEPuckModel::CalculateBoundingBox() {
   ...
   void CDynamics3DNewEPuckModel::UpdateEntityStatus() {
   ...
   void CDynamics3DNewEPuckModel::UpdateFromEntityStatus() {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DNewEPuckModel::AddToWorld(btMultiBodyDynamicsWorld& c_world) {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DNewEPuckModel::RemoveFromWorld(btMultiBodyDynamicsWorld& c_world) {
   ...
   REGISTER_STANDARD_DYNAMICS3D_OPERATIONS_ON_ENTITY(CNewEPuckEntity, CDynamics3DNewEPuckModel);

   /****************************************/
   /****************************************/

   const btVector3    CDynamics3DNewEPuckModel::m_cBodyHalfExtents(0.0362, 0.0236, 0.0362);
   const btScalar     CDynamics3DNewEPuckModel::m_fBodyMass(0.242);
   const btTransform  CDynamics3DNewEPuckModel::m_cBodyOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,0.00125,0.0));
   const btTransform  CDynamics3DNewEPuckModel::m_cBodyGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0, -0.0236, 0.0));
   const btVector3    CDynamics3DNewEPuckModel::m_cWheelHalfExtents(0.02125,0.0015,0.02125);
   const btScalar     CDynamics3DNewEPuckModel::m_fWheelMass(0.006);
   const btTransform  CDynamics3DNewEPuckModel::m_cWheelGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,-0.0015,0.0));
   const btTransform  CDynamics3DNewEPuckModel::m_cLeftWheelOffset(btQuaternion(btVector3(-1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, -0.0255));
   const btTransform  CDynamics3DNewEPuckModel::m_cRightWheelOffset(btQuaternion(btVector3(1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, 0.0255));
   const btVector3    CDynamics3DNewEPuckModel::m_cBodyToRightWheelJointOffset(0.0, -0.0036, 0.0255);
   const btVector3    CDynamics3DNewEPuckModel::m_cRightWheelToBodyJointOffset(0.0, 0.0015, 0.0);
   const btQuaternion CDynamics3DNewEPuckModel::m_cBodyToRightWheelJointRotation(btVector3(-1,0,0), SIMD_HALF_PI);
   const btVector3    CDynamics3DNewEPuckModel::m_cBodyToLeftWheelJointOffset(0.0, -0.0036, -0.0255);
   const btVector3    CDynamics3DNewEPuckModel::m_cLeftWheelToBodyJointOffset(0.0, 0.0015, -0.0);
   const btQuaternion CDynamics3DNewEPuckModel::m_cBodyToLeftWheelJointRotation(btVector3(1,0,0), SIMD_HALF_PI);
   /* TODO calibrate these values */
   const btScalar     CDynamics3DNewEPuckModel::m_fWheelMotorMaxImpulse(0.15);
   const btScalar     CDynamics3DNewEPuckModel::m_fWheelFriction(5.0);
