/**
 * @file <argos3/plugins/simulator/sensors/newepuck_proximity_default_sensor.cpp>
 *
...
namespace argos {
   ...
   void CNewEPuckProximityDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_NewEPuckProximitySensor::Init(t_tree);
         /* Show rays? */
   ...
   Real CNewEPuckProximityDefaultSensor::CalculateReading(Real f_distance)
   {
      //return Exp(-f_distance);

      // from the new_e-puck model by Lorenzo Garattoni and Gianpiero Francesca
      Real value = 0.0f;
