...
namespace argos {
   ...
   CDynamics3DTurtlebot4Model::CDynamics3DTurtlebot4Model(CDynamics3DEngine& c_engine,
                                                CTurtlebot4Entity& c_turtlebot4) :
      /* technically, the CDynamics3DMultiBodyObjectModel should be initialized with 7 children
         links, however, setting it to 7 makes the turtlebot4 less stable for reasons. */
      CDynamics3DMultiBodyObjectModel(c_engine, c_turtlebot4, 3, false),
      m_cWheeledEntity(c_turtlebot4.GetWheeledEntity()) {
      ...
      const SAnchor& sOriginAnchor = c_turtlebot4.GetEmbodiedEntity().GetOriginAnchor();
      ...
      SAnchor* psBodyAnchor = &c_turtlebot4.GetEmbodiedEntity().AddAnchor("body", {0.0, 0.0, 0.00125});
      /* create the bodies */
   ...
   void CDynamics3DTurtlebot4Model::Reset() {
   ...
   void CDynamics3DTurtlebot4Model::CalculateBoundingBox() {
   ...
   void CDynamics3DTurtlebot4Model::UpdateEntityStatus() {
   ...
   void CDynamics3DTurtlebot4Model::UpdateFromEntityStatus() {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DTurtlebot4Model::AddToWorld(btMultiBodyDynamicsWorld& c_world) {
      /* run the base class's implementation of this method */
   ...
   void CDynamics3DTurtlebot4Model::RemoveFromWorld(btMultiBodyDynamicsWorld& c_world) {
   ...
   REGISTER_STANDARD_DYNAMICS3D_OPERATIONS_ON_ENTITY(CTurtlebot4Entity, CDynamics3DTurtlebot4Model);

   /****************************************/
   /****************************************/

   const btVector3    CDynamics3DTurtlebot4Model::m_cBodyHalfExtents(0.0362, 0.0236, 0.0362);
   const btScalar     CDynamics3DTurtlebot4Model::m_fBodyMass(0.242);
   const btTransform  CDynamics3DTurtlebot4Model::m_cBodyOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,0.00125,0.0));
   const btTransform  CDynamics3DTurtlebot4Model::m_cBodyGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0, -0.0236, 0.0));
   const btVector3    CDynamics3DTurtlebot4Model::m_cWheelHalfExtents(0.02125,0.0015,0.02125);
   const btScalar     CDynamics3DTurtlebot4Model::m_fWheelMass(0.006);
   const btTransform  CDynamics3DTurtlebot4Model::m_cWheelGeometricOffset(btQuaternion(0.0, 0.0, 0.0, 1.0), btVector3(0.0,-0.0015,0.0));
   const btTransform  CDynamics3DTurtlebot4Model::m_cLeftWheelOffset(btQuaternion(btVector3(-1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, -0.0255));
   const btTransform  CDynamics3DTurtlebot4Model::m_cRightWheelOffset(btQuaternion(btVector3(1,0,0), SIMD_HALF_PI), btVector3(0.0, 0.02125, 0.0255));
   const btVector3    CDynamics3DTurtlebot4Model::m_cBodyToRightWheelJointOffset(0.0, -0.0036, 0.0255);
   const btVector3    CDynamics3DTurtlebot4Model::m_cRightWheelToBodyJointOffset(0.0, 0.0015, 0.0);
   const btQuaternion CDynamics3DTurtlebot4Model::m_cBodyToRightWheelJointRotation(btVector3(-1,0,0), SIMD_HALF_PI);
   const btVector3    CDynamics3DTurtlebot4Model::m_cBodyToLeftWheelJointOffset(0.0, -0.0036, -0.0255);
   const btVector3    CDynamics3DTurtlebot4Model::m_cLeftWheelToBodyJointOffset(0.0, 0.0015, -0.0);
   const btQuaternion CDynamics3DTurtlebot4Model::m_cBodyToLeftWheelJointRotation(btVector3(1,0,0), SIMD_HALF_PI);
   /* TODO calibrate these values */
   const btScalar     CDynamics3DTurtlebot4Model::m_fWheelMotorMaxImpulse(0.15);
   const btScalar     CDynamics3DTurtlebot4Model::m_fWheelFriction(5.0);
