...
namespace argos {
   ...
   const Real TURTLEBOT4_BASE_ELEVATION = 0.009;
   const Real TURTLEBOT4_BASE_RADIUS    = 0.08;
   const Real TURTLEBOT4_BASE_HEIGHT    = 0.192;
   ...
   const CRange<Real> TURTLEBOT4_LIDAR_SENSORS_RING_RANGE;
   ...
   void CTurtlebot4LIDARDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the Turtlebot4 LIDAR default sensor", ex);
      }
   }
   ...
   void CTurtlebot4LIDARDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         CCI_Turtlebot4LIDARSensor::Init(t_tree);
         /* How many readings? */
         GetNodeAttributeOrDefault(t_tree, "num_readings", m_unNumReadings, m_unNumReadings);
         m_pcProximityEntity->AddSensorFan(
            CVector3(0.0, 0.0, TURTLEBOT4_LIDAR_ELEVATION),
            TURTLEBOT4_LIDAR_SENSORS_FAN_RADIUS + TURTLEBOT4_LIDAR_SENSORS_RING_RANGE.GetMin(),
            -TURTLEBOT4_LIDAR_ANGLE_SPAN * 0.5,
            TURTLEBOT4_LIDAR_ANGLE_SPAN * 0.5,
   ...
   void CTurtlebot4LIDARDefaultSensor::Update() {
      /* Nothing to do if sensor is deactivated */
      if(m_unPowerLaserState != TURTLEBOT4_POWERON_LASERON)
         return;
   ...
   void CTurtlebot4LIDARDefaultSensor::Reset() {
   ...
   void CTurtlebot4LIDARDefaultSensor::Destroy() {
      delete[] m_pnReadings;
   }
   ...
   size_t CTurtlebot4LIDARDefaultSensor::GetNumReadings() const {
   ...
   void CTurtlebot4LIDARDefaultSensor::PowerOn() {
      m_unPowerLaserState = m_unPowerLaserState | 0x1;
      m_pcProximityEntity->SetEnabled(m_unPowerLaserState == TURTLEBOT4_POWERON_LASERON);
   }

   /****************************************/
   /****************************************/

   void CTurtlebot4LIDARDefaultSensor::PowerOff() {
   ...
   void CTurtlebot4LIDARDefaultSensor::LaserOn() {
   ...
   void CTurtlebot4LIDARDefaultSensor::LaserOff() {
