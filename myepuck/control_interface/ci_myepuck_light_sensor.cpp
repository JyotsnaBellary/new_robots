...
#include "ci_myepuck_light_sensor.h"
...
namespace argos {
   ...
   CCI_MyEPuckLightSensor::CCI_MyEPuckLightSensor() :
      m_tReadings(24) {
   ...
   const CCI_MyEPuckLightSensor::TReadings& CCI_MyEPuckLightSensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_MyEPuckLightSensor::CreateLuaState(lua_State* pt_lua_state) {
...
#else
   void CCI_MyEPuckLightSensor::CreateLuaState(void*) {}
#endif
...
#ifdef ARGOS_WITH_LUA
   void CCI_MyEPuckLightSensor::ReadingsToLuaState(lua_State* pt_lua_state) {
...
#else 
   void CCI_MyEPuckLightSensor::ReadingsToLuaState(void*) {}
#endif
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_MyEPuckLightSensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_MyEPuckLightSensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
