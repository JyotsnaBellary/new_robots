/**
 * @file <myepuck/simulator/myepuck_light_rotzonly_sensor.cpp>
 *
...
#include "myepuck_light_rotzonly_sensor.h"
...
namespace argos {
   ...
   CMyEPuckLightRotZOnlySensor::CMyEPuckLightRotZOnlySensor() :
   ...
   void CMyEPuckLightRotZOnlySensor::SetRobot(CComposableEntity& c_entity) {
      try {
         ...
         THROW_ARGOSEXCEPTION_NESTED("Can't set robot for the my_e-puck light default sensor", ex);
      }
   }
   ...
   void CMyEPuckLightRotZOnlySensor::Init(TConfigurationNode& t_tree) {
      try {
         /* Show rays? */
   ...
   void CMyEPuckLightRotZOnlySensor::Update() {
      ...
      CRadians cAngleLightWrtMyEpuck;
      ...
      for(auto it = mapLights.begin();
         ...
         if(cLight.GetIntensity() > 0.0f) {
            ...
            if(! GetClosestEmbodiedEntityIntersectedByRay(sIntersection,
               ...
               cAngleLightWrtMyEpuck = cRobotToLight.GetZAngle();
               cAngleLightWrtMyEpuck -= cOrientationZ;
               /*
                * Find closest sensor index to point at which ray hits myepuck body
                * Rotate whole body by half a sensor spacing (corresponding to placement of first sensor)
                * Division says how many sensor spacings there are between first sensor and point at which ray hits myepuck body
                * Increase magnitude of result of division to ensure correct rounding
                */
               Real fIdx = (cAngleLightWrtMyEpuck - SENSOR_HALF_SPACING) / SENSOR_SPACING;
               ...
               for(SInt32 nIndexOffset = -6; nIndexOffset < 7; ++nIndexOffset) {
                  ...
                  CRadians cAngularDistanceFromOptimalLightReceptionPoint = Abs((cAngleLightWrtMyEpuck - m_tReadings[unIdx].Angle).SignedNormalize());
                  /*
   ...
   void CMyEPuckLightRotZOnlySensor::Reset() {
   ...
   REGISTER_SENSOR(CMyEPuckLightRotZOnlySensor,
                   "myepuck_light", "rot_z_only",
                   "Carlo Pinciroli [ilpincy@gmail.com]",
                   "1.0",
                   "The my_e-puck_light light sensor (optimized for 2D).",
                   ...
                   "      <sensors>\n"
                   "        ...\n"
                   "        <myepuck_light implementation=\"rot_z_only\" />\n"
                   "        ...\n"
                   "      </sensors>\n"
                   "      ...\n"
                   "    </my_controller>\n"
